...
public class Sistema
{
    ...
    public static boolean crearGrafoCompleto()
    {
        try
        {
            grafoCompleto = new GrafoLocalidadCompleto(new LinkedList<Localidad>(localidadesCargadas), costoPorKm, porcentajeSupera300Km, costoProvinciasDistintas);
    ...
    {
        if (grafo != null)
            grafoCompleto = new GrafoLocalidadCompleto(new LinkedList<Localidad>(grafo.getLocalidades()), costoPorKm, porcentajeSupera300Km, costoProvinciasDistintas);
    }
    ...
    public static double getPrecioBase(PRECIOS precioPedido)
    {
        if (precioPedido == null)
            throw new IllegalArgumentException("Debe pedir un precio dentro de el SISTEMA. PRECIOS.PORKILOMETOS, PRECIOS.PORCENTAJESUPERA300KM, PRECIOS.DOSPROVINCIASDISTINTAS");
    ...
    {
        if (precio < 0)
            throw new IllegalArgumentException("El costo fijo agregado por provincias distintas no puede ser negativo.");
    ...
    public static boolean cambiarAristaPorOtra(Localidad origen, Localidad destinoOriginal, Localidad nuevoDestino)
    {
        ...
        if (grafoAux.existeArista(origen, destinoOriginal) && grafoAux.existeVertice(nuevoDestino)&& !grafoAux.existeArista(origen, nuevoDestino))
        {
